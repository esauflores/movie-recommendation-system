...
class MovieEmbeddingOpenAI(Base):
    ...
    embedding_ada_002: Mapped[list[float]] = mapped_column(
        VECTOR(1536), nullable=False
    )  # OpenAI's embedding size is 1536

    embedding_3_small: Mapped[list[float]] = mapped_column(
        VECTOR(1536), nullable=True
    )  # OpenAI's text-embedding-3-small

    embedding_3_large: Mapped[list[float]] = mapped_column(
